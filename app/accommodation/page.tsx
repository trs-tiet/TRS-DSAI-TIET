    ...
    <section className="bg-paper py-24 min-h-[60vh]">
      <div className="wrap">
        ...
        <div className="flex flex-col gap-px bg-line mb-16">
          {nearbyHotels.map((h, i) => (
            <Reveal key={h.name} delay={i * 0.06}>
              <div className="bg-white p-6 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-8">
                <div className="font-medium text-[15px] sm:w-64 shrink-0">{h.name}</div>
                <div className="text-[14px] text-steel">{h.note}</div>
              </div>
            </Reveal>
          ))}
        </div>
                ...
                <div className="flex flex-col gap-4">
                  {section.routes.map((r) => (
                    <div key={r.mode}>
                      ...
                      <div className="text-[13.5px] text-steel">{r.detail}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <p className="text-[13.5px] text-steel mt-10 max-w-lg">
          Distances are approximate — edit{" "}
          <code className="bg-line px-1.5 py-0.5 rounded-sm">data/accommodation.ts</code> to
          update contact details, hotel names, or add booking links once finalized.
        </p>
      </div>
    </section>
  );
